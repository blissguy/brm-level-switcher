...
= 1.1.2 =
* Flush the persistent object cache on each level switch so the new level is reflected on the very next front-end render instead of waiting out BricksMembers' object-cache TTL (~15 min). Previously the full-page cache-bust alone wasn't enough because BRM's per-post enrollment/drip/access snapshots stayed cached. The flush runs only on the gated, nonce-checked admin switch action. Admin/dev-only tool, so the all-users flush is acceptable.
